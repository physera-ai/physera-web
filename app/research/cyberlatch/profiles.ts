...
export const profiles: Profile[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "A single zero is its largest miss. On one injection task the agent wrote its own regression tests into the workspace and left them there. A preflight hygiene check rejected the submission before any of the 65 real checks ran. Its other large miss was in access control, where the repair stopped both tenants from publishing legitimately while still accepting a staging credential for publish.",
  ...
  },
  {
    ...
    body: [
      ...
      "It also carries a recorded zero on one authentication task after the verifier failed closed. The agent rejected freshly issued codes on their first use, so no replay test ever reached a successful request. Elsewhere its misses were small and two runs stopped one check short.",
    ],
  },
  {
    key: "grok-4.6",
    ...
    body: [
      ...
      "Authentication and investigation were weaker. One authentication run is recorded as zero after the verifier failed closed; the first request was allowed and both replays were accepted. It solved none of the three investigations and left the evidence and remediation sections incomplete in one of them.",
    ],
  },
  {
    key: "kimi-k3",
  ...
  },
  {
    ...
    body: [
      ...
      "Its single near miss was an investigation at 9 of 10, where only a phase-one schema check failed. One authentication run is recorded as zero after the verifier failed closed, with both replay attempts accepted.",
    ],
  },
];
