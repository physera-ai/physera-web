  ...
  return (
    <main className="min-h-screen bg-(--site-bg) text-(--site-fg) relative selection:bg-white selection:text-black transition-colors duration-300">
      ...
      <div className="relative z-10 flex flex-col min-h-screen pointer-events-none">
        ...
        <div className="flex-1 flex flex-col items-center px-5 sm:px-4 pt-40 pb-20 pointer-events-auto cursor-default">
          <div className="w-full max-w-[700px] flex flex-col gap-12 relative z-10 text-left">
            <InteractiveHeroTitle>
              Team
            </InteractiveHeroTitle>
            
            <div className="flex flex-col font-sans text-[16px] sm:text-[18px]">
              {teamMembers.map((member) => (
                <div key={member.name} className="flex flex-row items-center justify-between gap-4 py-6 border-b border-(--island-rule) first:border-t">
                  <span className="text-(--site-fg) font-medium">{member.name}</span>
                  <a 
                    href={member.x} 
                    target="_blank" 
                    rel="noopener noreferrer" 
                    className="text-(--site-fg-muted) hover:text-(--site-fg) transition-colors flex items-center gap-2 group"
                  >
                    X
                    <svg className="h-[12px] w-[12px] fill-current opacity-70 group-hover:opacity-100 transition-opacity" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
