...
export function HeaderSideways() {
  const [isOpen, setIsOpen] = useState(false);

  return (
    <header
      className={`fixed left-2 sm:left-4 top-2 sm:top-4 z-50 h-11 sm:h-12 overflow-hidden rounded-[5px] text-black transition-[max-width] duration-[400ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${
        isOpen ? "max-w-[650px]" : "max-w-[220px] sm:max-w-[240px]"
      }`}
    >
      <svg className="absolute inset-0 -z-10 h-full w-full pointer-events-none" preserveAspectRatio="none">
        <defs>
          <mask id="headersideways-mask">
            <rect width="100%" height="100%" fill="white" />
            
            {/* Top cutout at x=228 (240 - 12 to center it) for sm, x=208 for mobile */}
            <svg x="calc(240px - 12px)" y="0" width="24" height="8" overflow="visible" className="hidden sm:block">
              <path
                d="M 0 0 H 5 A 2 2 0 0 1 7 2 A 5 5 0 0 0 17 2 A 2 2 0 0 1 19 0 H 24 V -1 H 0 Z"
                fill="black"
            ...
            <svg x="calc(220px - 12px)" y="0" width="24" height="8" overflow="visible" className="block sm:hidden">
              <path
                d="M 0 0 H 5 A 2 2 0 0 1 7 2 A 5 5 0 0 0 17 2 A 2 2 0 0 1 19 0 H 24 V -1 H 0 Z"
                fill="black"
                style={{ transformOrigin: '12px 0px' }}
            ...
            {/* Bottom cutouts */}
            <svg x="calc(240px - 12px)" y="calc(100% - 8px)" width="24" height="8" overflow="visible" className="hidden sm:block">
              <path
                d="M 0 8 H 5 A 2 2 0 0 0 7 6 A 5 5 0 0 1 17 6 A 2 2 0 0 0 19 8 H 24 V 9 H 0 Z"
                fill="black"
            ...
            <svg x="calc(220px - 12px)" y="calc(100% - 8px)" width="24" height="8" overflow="visible" className="block sm:hidden">
              <path
                d="M 0 8 H 5 A 2 2 0 0 0 7 6 A 5 5 0 0 1 17 6 A 2 2 0 0 0 19 8 H 24 V 9 H 0 Z"
                fill="black"
                style={{ transformOrigin: '12px 8px' }}
        ...
        <rect width="100%" height="100%" fill="white" mask="url(#headersideways-mask)" />
      </svg>
      
      <div className="flex h-full w-[650px] items-center">
        {/* Original Header Part (Logo + Hamburger) */}
        <div className="flex h-full w-[220px] sm:w-[240px] shrink-0 items-center justify-between pl-4 pr-1">
          <Link
          ...
          >
            <span className="relative h-[14px] w-[22px] transition-opacity duration-300">
              <span
                className={`absolute left-0 top-0 h-[2px] w-full rounded-full bg-black transition-all duration-[400ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${
                  isOpen ? "translate-y-[6px] rotate-45" : ""
                }`}
              />
              <span
                className={`absolute left-0 top-[6px] h-[2px] w-full rounded-full bg-black transition-all duration-[400ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${
                  isOpen ? "scale-x-0 opacity-0" : ""
                }`}
              />
              <span
                className={`absolute left-0 top-[12px] h-[2px] w-full rounded-full bg-black transition-all duration-[400ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${
                  isOpen ? "-translate-y-[6px] -rotate-45" : ""
                }`}
              />
            </span>
        ...
        {/* Separator Line */}
        <div className={`flex h-full items-center justify-center transition-opacity duration-[400ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${isOpen ? "opacity-100" : "opacity-0"}`}>
           <span className="h-6 w-px bg-black/10" />
        </div>

        {/* Expanded Part */}
        <div className={`flex h-full items-center px-4 transition-opacity duration-[400ms] ease-[cubic-bezier(0.16,1,0.3,1)] delay-100 ${isOpen ? "opacity-100" : "opacity-0"}`}>
          <nav className="flex items-center gap-1 sm:gap-2">
            ...
            <a
              className="flex items-center gap-2 px-2 sm:px-3 py-1.5 font-display text-[15px] sm:text-[16px] font-medium tracking-[-0.32px] text-black/70 transition-colors hover:text-black"
