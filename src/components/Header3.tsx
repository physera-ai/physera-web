...
export function Header3({ position = 'center' }: { position?: 'center' | 'sides' }) {
  const containerClass = position === 'center' 
    ? 'fixed left-0 right-0 top-2 sm:top-4 z-50 flex justify-center items-start gap-1 sm:gap-2 pointer-events-none px-2 sm:px-4'
    : 'fixed left-0 right-0 top-2 sm:top-4 z-50 flex justify-between items-start pointer-events-none px-2 sm:px-4';

  return (
    <div className={containerClass}>
      <header
        className="pointer-events-auto relative h-11 sm:h-8 overflow-hidden rounded-[5px] text-black w-max max-w-[calc(100vw-130px)] sm:max-w-none"
      >
        <svg className="absolute inset-0 -z-10 h-full w-full pointer-events-none" preserveAspectRatio="none">
          <defs>
            <mask id="header3-mask">
              <rect width="100%" height="100%" fill="white" />
              
              {/* Top Left cutout */}
              <svg x="-4" y="-8" width="16" height="16" overflow="visible">
                <path
                  d="M 0 8 A 4 4 0 0 0 8 8 A 4 4 0 0 0 0 8 Z"
                  fill="black"
                />
              </svg>
              ...
              <svg x="-4" y="calc(100% - 8px)" width="16" height="16" overflow="visible">
                <path
                  d="M 0 8 A 4 4 0 0 1 8 8 A 4 4 0 0 1 0 8 Z"
                  fill="black"
                />
              </svg>
              ...
              <svg x="calc(170px - 4px)" y="-8" width="16" height="16" overflow="visible" className="hidden sm:block">
                <path
                  d="M 0 8 A 4 4 0 0 0 8 8 A 4 4 0 0 0 0 8 Z"
                  fill="black"
                />
              </svg>
              <svg x="calc(220px - 4px)" y="-8" width="16" height="16" overflow="visible" className="block sm:hidden">
                <path
                  d="M 0 8 A 4 4 0 0 0 8 8 A 4 4 0 0 0 0 8 Z"
                  fill="black"
                />
              ...
              {/* Bottom separator cutouts */}
              <svg x="calc(170px - 4px)" y="calc(100% - 8px)" width="16" height="16" overflow="visible" className="hidden sm:block">
                <path
                  d="M 0 8 A 4 4 0 0 1 8 8 A 4 4 0 0 1 0 8 Z"
                  fill="black"
                />
              </svg>
              <svg x="calc(220px - 4px)" y="calc(100% - 8px)" width="16" height="16" overflow="visible" className="block sm:hidden">
                <path
                  d="M 0 8 A 4 4 0 0 1 8 8 A 4 4 0 0 1 0 8 Z"
                  fill="black"
                />
          ...
          <rect width="100%" height="100%" fill="white" mask="url(#header3-mask)" />
        </svg>
      ...
      <button className="pointer-events-auto cursor-pointer relative h-11 sm:h-8 overflow-hidden rounded-[5px] flex items-center px-4 sm:px-5 transition-opacity hover:opacity-80 shrink-0 group">
        <svg className="absolute inset-0 -z-10 h-full w-full pointer-events-none" preserveAspectRatio="none">
          <defs>
          ...
          <rect width="100%" height="100%" fill="white" mask="url(#waitlist-mask)" className="transition-colors group-hover:fill-[#f9f9f9]" />
        </svg>
        <span className="font-display text-[14px] sm:text-[15px] font-medium text-[#FF4400] tracking-[-0.01em]">
          Join Waitlist
        </span>
      </button>
    </div>
